...
my %XMLnd;      # GAPs from XMLs
my %XMLblast;   # BLASTSs from XMLs
my %XMLmicro;   # Microarray data from XMLs
...
my %blastKeyRev;   # gene id > gene name
...
sub start
{
	...
	print "  MERGING DATA...";
	&mergeData(\%XMLsnp, \%XMLgap, \%XMLblast, \%XMLmicro, \%XMLnd, $inputFA);
	print "done\n";	
...
# 			$max = $pos if ($pos > $max);
	...
	foreach my $table (keys %XMLnd)
	{
		foreach my $register (keys %{$XMLnd{$table}})
		{
			undef $prog;
			undef $chrom;
			undef @prog;
...
# 
			...
			if ($tableType eq "chromossome")
			{
				$prog     = $XMLnd{$table}{$register}{"program"};
				$chrom    = $table;
				$chromNum = $chromos{$chrom};
			}
			elsif ($tableType eq "program")
			{
				$prog     = $table;
				$chrom    = $XMLnd{$table}{$register}{"chromR"};
				$chromNum = $chromos{$chrom};
			}
			else
			{
			...
			my $start  = $XMLnd{$table}{$register}{"start"};
			my $end    = $XMLnd{$table}{$register}{"end"};
			my $length = $XMLnd{$table}{$register}{"length"};
			my $reads  = $XMLnd{$table}{$register}{"reads"};
			my $rLeft  = $XMLnd{$table}{$register}{"rLeft"};
			my $rRight = $XMLnd{$table}{$register}{"rRight"};
			my $id     = $XMLnd{$table}{$register}{"id"};
			...
			push (@{$stat{$chromNum}{"ND_prog_pos"}{$prog}}, $start);
			my %hash;
			   %hash = %{$XMLpos[$chromNum][$start]} if ( defined %{$XMLpos[$chromNum][$start]});
			...
			for (my $i = ($start+1); $i <= $end; $i++)
			{
				push (@{$stat{$chromNum}{"ND_prog_pos_total"}{$prog}}, $i);
			}

			my $chromName = &reverseHash($chromNum);
			print MERGED "$chromName >> $start > nd = $indel\n";
			print MERGED "$chromName >> $start > nd_end = $end\n";
# 			$max = $pos if ($pos > $max);
		} #end foreach my register
	} #end foreach my table
	undef %unique;
...
# 
			...
			if ( ! defined $blastKey{$id} ) { $blastKey{$id} = $gene; $blastKeyRev{$gene} = $id;};
			...
			if ( defined %{$XMLpos[$chromNum][$start]})
			{
				while (defined $XMLpos[$chromNum][$start]->{"gene"}) { $start++; };
				if ( defined %{$XMLpos[$chromNum][$start]})
				{
					%hash = %{$XMLpos[$chromNum][$start]};
				}
# 				print "$chromNum $start defined\n";
# 				print "OLD " . $hash{"gene"} . " > " . $blastKey{$hash{"gene"}} . "\nNEW $id > $gene\n";
			}; 
...
# 			$max = $pos if ($pos > $max);
	...
	foreach my $table (keys %XMLmicro)
	{
 		#print "\tTABLE $table\n";
		foreach my $register (keys %{$XMLmicro{$table}})
		{
 			#print "\t\tREGISTER $register\n";
			$total++;
			undef $prog;
			...
			#<end>4013</end>
			#<expLev>-2.49</expLev>
			#<gene>114236_GE_CneoSBc1_1_6000_s_RC-60-1988_FOLDS_-2.49</gene>
			#<spotId>GE_CneoSBc1_1_6000_s_RC-60-1988</spotId>
			#<spotPos>114236</spotPos>
			#<start>3954</start>
			#<strand>-1</strand>


			my $start   = $XMLmicro{$table}{$register}{"start"};
			my $end     = $XMLmicro{$table}{$register}{"end"};
			my $strand  = $XMLmicro{$table}{$register}{"strand"};
			my $spotId  = $XMLmicro{$table}{$register}{"spotId"};
			my $expLev  = $XMLmicro{$table}{$register}{"expLev"};
			my $corr    = $XMLmicro{$table}{$register}{"correlation"};
			my $gene    = $XMLmicro{$table}{$register}{"gene"};
			my $spotPos = $XMLmicro{$table}{$register}{"spotPos"};
			my $id      = $XMLmicro{$table}{$register}{"id"};

			if ( ! ((defined $start) && (defined $end) && (defined $strand) && (defined $spotId) && ((defined $expLev) || (defined $corr)) && (defined $spotPos) && (defined $gene)))
			{
				die "COULD NOT GET START ($start), END ($end), STRAND ($strand), SPOTID ($spotId), EXPRESSION LEVEL($expLev), CORRELATION($corr), SPOT POSITION($spotPos) OR MICROARRAY FULL NAME ($gene) FROM TABLE $table REGISTER NUMBER $register ID $gene";
			}
			...
			if ( defined %{$XMLpos[$chromNum][$start]})
			{
				while (defined $XMLpos[$chromNum][$start]->{"microarray"}) { $start++; };
				if ( defined %{$XMLpos[$chromNum][$start]})
				{
					%hash = %{$XMLpos[$chromNum][$start]};
				}
...
# 				print "OLD " . $hash{"gene"} . " > " . $blastKey{$hash{"gene"}} . "\nNEW $id > $gene\n";
			...
			#$hash{"microarray"}        = $id;
			$hash{"microarray_end"}    = $end;
			if ($expLev) { $hash{"microarray_exp"}    = $expLev; };
			if ($corr)   { $hash{"microarray_cor"}    = $corr; };
			#$hash{"microarray_id"}     = $spotId;
			$hash{"microarray_id"}     = $gene;
			$XMLpos[$chromNum][$start] = \%hash;
...
# 			4718    273944K 173.800K 99      2.1     0:12.92 xml_pos.pl
			...
			my $chromName = &reverseHash($chromNum);
			print MERGED "$chromName >> $start > microarray     = $gene\n";
			print MERGED "$chromName >> $start > microarray_end = $end\n";
			print MERGED "$chromName >> $start > microarray_exp = $expLev\n" if ($expLev);
			print MERGED "$chromName >> $start > microarray_cor = $corr\n" if ($corr);
			#print MERGED "$chromName >> $start > microarray_id  = $spotId\n";

# 			$max = $pos if ($pos > $max);
		} #end foreach my register
	} #end foreach my table
	print "$total MICROARRAYS OUT\n";
...
sub getStat
{
	...
	if ((defined $stat{$chromNum}) && (keys %{$stat{$chromNum}}))
	{
		...
		if (defined %{$hash{"ND_prog_pos"}})
		{
			my %ND_prog_pos   = %{$hash{"ND_prog_pos"}}; # $prog > @pos	
			my %ND_prog_pos_count; # $prog > @pos	
			my @ND_prog       = (keys %{$hash{"ND_prog_pos"}}); # $prog > @pos	
			my @ND_pos;     #positions of the SNPs
			foreach my $prog (@ND_prog) { push(@ND_pos, @{$ND_prog_pos{$prog}}); };
			my %seen           = ();
			my @sorted         = grep  { ! $seen{$_} ++ } @ND_pos;
			   @ND_pos        = (sort { $a <=> $b} @sorted);

			foreach my $pro ( keys %ND_prog_pos)
			{
				my @proPos                = @{$ND_prog_pos{$pro}};
				my $programsP             = &positionTable(\@proPos);
				$ND_prog_pos{$pro}       = $programsP;
				$ND_prog_pos_count{$pro} = @proPos;
			}

			my $ND_count_u    =   0;

			foreach my $po (@ND_pos)
			{
				$positions_both[$po]++;
				$ND_count_u++;
			}
			...
			push(@progs, @ND_prog);
			push(@positions, @ND_pos);
			my $ND_prog  = &consensusProgSimple(\@ND_prog);
			my $ND_pos   = &positionTable(\@ND_pos);

			$response{"ND_prog"}  	 = $ND_prog;    # $Str
			$response{"ND_pos"}  	 = $ND_pos;     # $Str
			$response{"ND_prog_A"}	 = \@ND_prog;   # @Str
			$response{"ND_pos_A"}   = \@ND_pos;    # @Int
			$response{"ND_count_u"} = $ND_count_u; # $Int
			$response{"ND"} = 1;
		}
		else
		{
...
sub loadXML
{
	...
	elsif ($rootType eq "ND")
	{
		%XMLnd   = %{&mergeHash(\%XMLnd, \%XMLtemp)};
	}
...
# 		%XMLblast = %XMLtemp;
	...
	elsif ($rootType eq "microarray")
	{
		%XMLmicro = %{&mergeHash(\%XMLmicro, \%XMLtemp)};
# 		%XMLblast = %XMLtemp;
	}
	else
	{
...
sub init
{
	...
	#	print "E.G.: ./xml_image.pl input.fasta input.snp.xml input.gap.xml  input.blast.xml\n";
		...
		print "E.G.: ./xml_pos.pl input.fasta input.gap.indelsa.xml input.blast.xml input.snp.xml\n";
