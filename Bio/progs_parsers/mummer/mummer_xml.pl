...
my $usage = "USAGE: $0 <OUTPUT DIR> <INPUT BASE DIR> <PREFIX BASE> <SUBDIR FILTER>";
die "$usage" if (@ARGV < 4);

my $outdir       = $ARGV[0];
my $dir_nucmer   = $ARGV[1];
my $prefix_base  = $ARGV[2];
my $subdirFilter = $ARGV[3];

die "DIR $outdir NOT FOUND\n$usage"             if  ( ! -d $outdir     );
die "DIR $outdir/$dir_nucmer NOT FOUND\n$usage" if  ( ! -d "$outdir/$dir_nucmer" );

print "RUNNING $0 $outdir $dir_nucmer $prefix_base $subdirFilter\n";
print "OUTDIR \"$outdir\" NUCMER \"$dir_nucmer\" PREFIXBASE \"$prefix_base\" SUBDIRFILTER \"$subdirFilter\"\n";
...
my @subdirs = sort &list_subdir($dir, $subdirFilter, $prefix_base);
...
sub parse_gap
{
	...
	if (@lines)
	{
		print "EXPORTING XML FILE: xml/ND_$name\_prog.xml\n";
		open  FILE, ">xml/ND_$name\_prog.xml" || die "COULD NOT OPEN XML FILE FOR FILENAME ND_$name\_prog: $!";
		print FILE  "<root id=\"mummer\" type=\"ND\">\n";
		...
		foreach my $line (@lines)
		{
			...
			#GAP	supercontig_1.03_cryptococcus_gattii_CBS7750	supercontig_1.03_of_Cryptococcus_neoformans_Serotype_B_R265	0010027	0010413	0000386
			#GAP	supercontig_1.10_cryptococcus_gattii_CBS7750	supercontig_1.10_of_Cryptococcus_neoformans_Serotype_B_R265	0159926	0160527	0000601
			#INS	supercontig_1.01_of_Cryptococcus_neoformans_Serotype_B_R265	supercontig_1.01_cryptococcus_gattii_CBS7750	0018866	0018965	0000099
			elsif( $line =~ /^(GAP|INS)\s+(\S+)\s+(\S+)\s+(\d+)\s+(\d+)\s+(\d+)/ )
			{
				my $type  = $1;
				my $ref   = $2;
				my $qry   = $3;
				...
				my $end   = $5;
				my $size  = $6;
				#print "TYPE $type REF $ref QRY $qry START $start END $end SIZE $size\n";
				if ($size > $minGapSize)
				{
					my $shortname = $name;
					   $shortname =~ s/\.gap2//gi;
					...
					$out1 .= "\t\t\t<id>     " . $id                 . "</id>\n";
					$out2 .= "\t\t\t<program>" . "mummer_$shortname" . "</program>\n";
					$out3 .= "\t\t\t<chromR> " . $ref                . "</chromR>\n";
					$out4 .= "\t\t\t<chromQ> " . $qry                . "</chromQ>\n";
					$out4 .= "\t\t\t<type>   " . $type               . "</type>\n";
					$out4 .= "\t\t</row>\n";
					print FILE "$out1$out3$out4";
					$chrom{$ref} .= "$out1$out2$out4";
				}
		...
		print "EXPORTING XML FILE: xml/ND_$name\_chrom.xml\n";
		open  FILE, ">xml/ND_$name\_chrom.xml" || die "COULD NOT OPEN XML FILE FOR FILENAME ND_$name\_chrom: $!";
		print FILE "<root id=\"mummer\" type=\"ND\">\n";
...
{
	my $dir            = $_[0];
	my $negativeFilter = $_[1];
	my $positiveFilter = $_[2];

 	print "openning dir \"$dir\" and searching for subdirs\n";

	opendir (DIR, "$dir") || die "CANT READ DIRECTORY $dir: $!\n";
	my @dirs = grep { (!/^\./) && -d "$dir/$_" && ( ! /$negativeFilter/ ) && ( /$positiveFilter/ ) } readdir(DIR);
