...
#todo = reformat to use id instead of start position
...
{
    if (( -f $inputFA ) && ( -f $inputPos ) && (($inputStat) && ( -f $inputStat)))
    {
        my $output;
        if ( ! $inputFA =~ /\/*([\w|\.]+)\.fasta/)
        {
    ...
    else
    {
        ...
        if (( $inputStat) && ( ! -f $inputStat)) { $exit .= "FILE $inputStat DOESNT EXISTS\n";};
        print $exit;
        exit 1; 
...
sub genGeneArray{
    foreach my $table (keys %XMLpos)
    {
        foreach my $register (keys %{$XMLpos{$table}})
        {
...
#                       print "CAR CRAAAAAAAAAASSSSSSSSSHHHHHHHHHHHH...$mixRegister $mixName\n";# $table $i $idname $iddname\n"
                    ...
                    {
                        if ( ! defined $blastKey{$register} ) { $blastKey{$register} = $gene; };
                        $geneArray[$table][$i] = $register;
                    }
                }
            }
...
sub fixStat
{
  ...
  {
    foreach my $id (sort {$a <=> $b} keys %{$stat{$chrom}})
    {
