...
if ($outputFolder  =~ /\/$/) { chop $outputFolder; };
   $outputFolder  .= "\_xml/";
$outputFolder      = $inFolder . "/." . $outputFolder;


$dbNameGlobal      =~ s/\//\_/;
$dbNameGlobal      =~ s/ /\_/;
if (substr($dbNameGlobal, -1) eq "_") { chop $dbNameGlobal };
$dbNameGlobal      = "$outputFolder$outputPrefix\_" . $dbNameGlobal . ".xml";
...
print "
INPUT  FOLDER : $inFolder
OUTPUT FOLDER : $outputFolder
OUTPUT PREFIX : $outputPrefix
DB NAME GLOBAL: $dbNameGlobal
";
...
sub exportDocInfo
{
	...
	foreach my $key (sort keys %$hash)
	{
		...
		if ( -f $fullPath )
		{
			printf "\tCONVERTING #%4d/%4d \"%-50s\" [%3d\.%1d MB]:\n", ++$count, $total, &trunc($fullPath,50), int($size), (($size-int($size))*10);
...
sub exportSearchXml
{
		...
		print "EXPORTING TO OUTPUT FOLDER $outputFolder PREFIX $outputPrefix $outputFolder$outputPrefix.$wcName.xml\n";
		my %pos;
	   
		open FILE, ">$outputFolder$outputPrefix.$wcName.xml" or die "COULD NOT OPEN $outputFolder$outputPrefix.$wcName.xml: $!";
		print FILE "<xml>\n";
		print FILE "\t<$wcName>\n";
		...
		open FILE, ">$outputFolder$outputPrefix.$keyName.xml" or die "COULD NOT OPEN $outputPrefix.$keyName.xml: $!";
		print FILE "<xml>\n";
